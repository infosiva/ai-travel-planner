...
test('primary CTA visible', async ({ page }) => { await page.goto('/'); await expect(page.locator('a,button').filter({hasText:/plan|trip|travel|explore|start/i}).first()).toBeVisible({timeout:10_000}); });
